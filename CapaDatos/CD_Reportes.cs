                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader()) {
                        while (dr.Read()) {
                            lista.Add(new ReporteProducto()
                            {
                                ...
                                PrecioCompra = Convert.ToDecimal(dr["Precio Compra"].ToString(),new CultureInfo("es-CL")).ToString("N", formato),
                                PrecioVenta = Convert.ToDecimal(dr["Precio Venta"].ToString(),new CultureInfo("es-CL")).ToString("N", formato)
                            });
                        }
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                try
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            var totalVenta = Convert.ToDecimal(dr["Total Venta"].ToString(), new CultureInfo("es-CL"));
                            ...
                            lista.Add(new ReporteVenta()
                            {
                                ...
                                TotalVenta = totalVenta.ToString("N", formato),
                                TotalAcumulado = totalAcumulado.ToString("N", formato)
                            });
                        }
