                            ...
                            if (doc.Element("DETALLE_COMPRA") != null)
                            {
                                rptDetalleCompra = (from dato in doc.Elements("DETALLE_COMPRA")
                                                    select new Compra()
                                                    {
                                                        ...
                                                        TotalCosto = Convert.ToDecimal(dato.Element("TotalCosto").Value,new CultureInfo("es-CL")),
                                ...
                                rptDetalleCompra.oListaDetalleCompra = (from producto in doc.Element("DETALLE_COMPRA").Element("DETALLE_PRODUCTO").Elements("PRODUCTO")
                                                                        select new DetalleCompra()
                                                                        {
                                                                            ...
                                                                            PrecioUnitarioCompra = Convert.ToDecimal(producto.Element("PrecioUnitarioCompra").Value, new CultureInfo("es-CL")),
                                                                            TotalCosto = Convert.ToDecimal(producto.Element("TotalCosto").Value, new CultureInfo("es-CL"))
                                                                        }).ToList();
                            }
                            else
                            {
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaCompra.Add(new Compra()
                        {
                            ...
                            TotalCosto = Convert.ToDecimal(dr["TotalCosto"].ToString(), new CultureInfo("es-CL"))
                        });
                    }
                    dr.Close();
