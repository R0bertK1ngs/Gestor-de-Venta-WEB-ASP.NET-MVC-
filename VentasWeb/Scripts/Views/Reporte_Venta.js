    ...
    jQuery.ajax({
        ...
        success: function (data) {
            if (data != undefined && data != null) {
                ...
                // Mostrar los datos
                $.each(data.Ventas, function (i, row) {
                    var totalVentaText = row["TotalVenta"];
                    var totalVenta = parseFloat(totalVentaText.replace(/\./g, '').replace(',', '.'));

                    if (!isNaN(totalVenta)) {
                        totalAcumulado += totalVenta;
                    } else {
                        console.log("Valor inválido para TotalVenta:", totalVentaText);
                    }
                ...
                $("#tbReporte").after(
                    ...
                    '<span id="totalAcumulado">' + data.TotalAcumulado.toLocaleString('es-CL', { minimumFractionDigits: 2 }) + '</span>' +
                    '</div>' +
                    '</div>'
                );
            }
